...
- Kyoko uses a complicated system to reply to most dialogue targeted at her (the Targeted>Responses dialogue folder) such as that she says her line a screen later if the character she’s responding to is on her right, so that the dialogue reads left-to-right. This is how the effect is achieved:
    1. Write the base version of the line, responding as normal and using its specific marker as normal. Give this line a variable test checking that the character is on Kyoko’s LEFT (<test expr="~CHARACTER.position~" value="left" />). This case should be in the Targeted>Responses>Stage (#) folder.
    2. Copy the base line, make it hidden, keep the marker, and change the variable test to instead check that the character is on Kyoko’s RIGHT. Move it to the Duplicate Dialogue folder.
    3. Copy the base line again, remove the marker, change any Saying Marker condition to be *Said* Marker instead, change the variable test to instead check that the character is on Kyoko’s RIGHT, move it to the Duplicate Dialogue folder, and change the dialogue trigger:
        - If the base line was for the Lost (Self) trigger, move this copy to the Stripping trigger.
        - If it was for the Stripping trigger, it should be After Stripping, and should be for the next stage.
        - If it was for the After Stripping trigger, it should be Hand=Any and a Self Time In Stage == 0 condition should be attached (so she will only say it the first hand after having just stripped).
    - If this is confusing for you, you are encouraged to filter Kyoko’s dialogue to a specific target (Nagatoro, Yumeko, and Revy are suggested as examples) and study the lines. It’s complicated and (as of December 2024) nobody else does it.
